...
testParallelConnectionsDetection = do
  let graph = build [Service "A" [Connection "B" HTTPS, Connection "B" FunctionCall]]
...
testSelfReferentialConnections = do
  let graph = build [Service "A" [Connection "A" HTTPS]]
  validationError <- assertLeft "Graph is not validated" $ validationToEither (checkGraph graph)
  assertEqual
...
testMismatchedConnections = do
  let graph = build [Service "A" [Connection "B" HTTPS], Service "B" [Connection "A" FunctionCall]]
  validationError <- assertLeft "Graph is not validated" $ validationToEither (checkGraph graph)
  assertEqual
