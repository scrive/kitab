...
testServiceDefinitionsParsing = do
  ...
  let expectedResult =
        ...
        , Service {serviceName = "s3", serviceInfo = ServiceInfo {serviceFqdn = Just "s3.amazonaws.com", serviceContext = Nothing}, connections = [], cidrSets = []}
        , Service {serviceName = "media-proxy", serviceInfo = ServiceInfo {serviceFqdn = Nothing, serviceContext = Just (ServiceContext {contextName = "k8s"})}, connections = [Connection {connectionWith = "opensearch", connectionType = HTTPS, connectionPorts = []}, Connection {connectionWith = "otel-tracing", connectionType = HTTPS, connectionPorts = [PortNode 4317 "TCP", PortNode 4318 "TCP"]}], cidrSets = []}
        ...
        , Service {serviceName = "main-app", serviceInfo = ServiceInfo {serviceFqdn = Nothing, serviceContext = Just (ServiceContext {contextName = "k8s"})}, connections = [Connection {connectionWith = "s3", connectionType = HTTPS, connectionPorts = []}, Connection {connectionWith = "media-proxy", connectionType = HTTPS, connectionPorts = []}, Connection {connectionWith = "user-registry", connectionType = FunctionCall, connectionPorts = []}, Connection {connectionWith = "otel-tracing", connectionType = HTTPS, connectionPorts = [PortNode 4317 "TCP"]}], cidrSets = []}
        ]
